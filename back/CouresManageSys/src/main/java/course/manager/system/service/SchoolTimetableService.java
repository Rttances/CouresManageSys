...
public class SchoolTimetableService extends ServiceImpl<SchoolTimetableMapper, SchoolTimetablePO> implements IService<SchoolTimetablePO> {
     ...
     * 分页条件查询课程表
     *
     * @param schoolTimetableROPageRO 分页条件查询参数
     * @return 查询到的课程表集合
     */
    public PageVO<SchoolTimetableVO> pageQuery(PageRO<SchoolTimetableRO> schoolTimetableROPageRO) {
        //TODO 后端查询代码冗余，年级专业学期字段为空值映射
        if (Objects.isNull(schoolTimetableROPageRO)) {
            throw ParamException.paramMissError("课程表分页条件查询参数为空");
        }
        SchoolTimetableRO schoolTimetableRO = schoolTimetableROPageRO.getEntity();
        if(Objects.isNull(schoolTimetableRO)){
            schoolTimetableRO = new SchoolTimetableRO();
        }
        ...
        Page<SchoolTimetablePO> schoolTimetablePOPage = baseMapper.selectPage(schoolTimetableROPageRO.toPage(), wrapper);

        List<SchoolTimetablePO> schoolTimetablePOS = baseMapper.selectList(wrapper);
        List<SchoolTimetableVO> schoolTimetableVOS = schoolTimetableInverter.po2VO(schoolTimetablePOS);
        if (CollUtil.isEmpty(schoolTimetableVOS)) {
            return PageVO.of(schoolTimetablePOPage, Lists.newArrayList());
        }
        Boolean queryVolunteerIs = schoolTimetableRO.getQueryVolunteerIs();
        Set<String> teacherIdSet = new HashSet<>();
        Set<String> courseIdSet = new HashSet<>();
        ...
        Map<String, String> courseIdMapCourseName = coursePOS.stream().collect(Collectors.toMap(CourseVO::getCourseId, CourseVO::getCourseName));
        // 填充专业年级学期
        Map<String, String> courseIdMapMajor = coursePOS.stream().collect(Collectors.toMap(CourseVO::getCourseId, CourseVO::getMajor));
        Map<String, Integer> courseIdMapGrade = coursePOS.stream().collect(Collectors.toMap(CourseVO::getCourseId, CourseVO::getGrade));
        Map<String, Integer> courseIdMapSemester = coursePOS.stream().collect(Collectors.toMap(CourseVO::getCourseId, CourseVO::getSemester));
        ...
        schoolTimetableVOS.forEach(schoolTimetableVO -> {
            ...
            if (StrUtil.isNotBlank(schoolTimetableVO.getCourseId())) {
                schoolTimetableVO.setCourseName(courseIdMapCourseName.getOrDefault(schoolTimetableVO.getCourseId(), Strings.EMPTY));
                schoolTimetableVO.setMajor(courseIdMapMajor.getOrDefault(schoolTimetableVO.getCourseId(), Strings.EMPTY));
                schoolTimetableVO.setGrade(courseIdMapGrade.getOrDefault(schoolTimetableVO.getCourseId(), 2023));
                schoolTimetableVO.setSemester(courseIdMapSemester.getOrDefault(schoolTimetableVO.getCourseId(), 1));
            }
            // 填充教师名称
            if (StrUtil.isNotBlank(schoolTimetableVO.getTeacherId())) {
                schoolTimetableVO.setTeacherName(teacherIdMapTeacherName.getOrDefault(schoolTimetableVO.getTeacherId(), Strings.EMPTY));
        ...
        return PageVO.of(schoolTimetablePOPage, schoolTimetableVOS);
    }
